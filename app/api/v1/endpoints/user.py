...
@router.post("/toggle-state", response_model=schemas.Response)
async def toggle_state(
    *,
    ...
    return {"message": "User state toggled successfully."}
...
async def create_user(
    *,
    ...
    user = await crud.user.create(db, obj_in=user_in)
    if settings.email_platform.EMAILS_ENABLED and user_in.email:
        send_new_account_email(email_to=user_in.email, username=user_in.email, password=user_in.password)
    return user


@router.get("/tester", response_model=schemas.Response)
async def test_endpoint() -> Any:
    """
    Test current endpoint.
    """
    return {"message": "Message returned ok."}