...
@router.put("/totp", response_model=schemas.Response)
async def enable_totp_authentication(
    *,
    ...
    current_user = await crud.user.update_totp_counter(db=db, db_obj=current_user, new_counter=new_counter)
    return {"message": "TOTP enabled. Do not lose your recovery code."}


@router.delete("/totp", response_model=schemas.Response)
...
@router.post("/revoke", response_model=schemas.Response)
...
@router.post("/recover/{email}", response_model=Union[schemas.WebToken, schemas.Response])
async def recover_password(email: str, db: AgnosticDatabase = Depends(deps.get_db)) -> Any:
    ...
    if user and crud.user.is_active(user):
        tokens = security.create_magic_tokens(subject=user.id)
        if settings.email_platform.EMAILS_ENABLED:
            send_reset_password_email(email_to=user.email, email=email, token=tokens[0])
            return {"claim": tokens[1]}
    return {"message": "If that login exists, we'll send you an email to reset your password."}


@router.post("/reset", response_model=schemas.Response)
